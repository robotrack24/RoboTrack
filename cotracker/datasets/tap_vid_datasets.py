...
class TapVidDataset(torch.utils.data.Dataset):
        ...
        if self.dataset_type == "kinetics":
            ...
            if not all_paths:
                raise FileNotFoundError(
                    f"No robotap_split*.pkl files under {data_root!r}. "
                    "Set dataset_root so that TAP-Vid RoboTAP lives at "
                    "dataset_root/tapvid_robotap/ or dataset_root/tapvid/tapvid_robotap/ "
                    "(see deepmind/tapnet for downloads)."
                )
            points_dataset = None
            for pickle_path in all_paths:
                with open(pickle_path, "rb") as f:
    ...
    def __getitem__(self, index):
        ...
        # NOTE: self.points_dataset is loaded once from a pickle and cached
        # in memory, so points/occluded are shared across __getitem__ calls.
        # Use out-of-place ops (or .copy()) when scaling, otherwise every
        # additional pass through the dataset (e.g. eval-every-N-steps during
        # training) re-multiplies the cached GT and silently drives Jaccard/
        # delta to ~0 while OA stays anchored at the GT-occlusion ratio.
        target_points = self.points_dataset[video_name]["points"]
        if self.resize_to is not None:
            frames = resize_video(frames, self.resize_to)
            target_points = target_points * np.array(
                [self.resize_to[1] - 1, self.resize_to[0] - 1]
            )  # 1 should be mapped to resize_to-1
        else:
            target_points = target_points * np.array(
                [frames.shape[2] - 1, frames.shape[1] - 1]
            )
