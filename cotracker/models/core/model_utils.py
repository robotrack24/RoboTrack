...
def get_sift_sampled_pts(
    ...
    if first_frame_only:
        ...
        nfeatures = size // num_sampled_frames

    sift = cv2.SIFT_create(nfeatures=nfeatures)
    B, T, C, H, W = video.shape

    all_points = list()
    max_t = min(T, max(1, int(num_frames * sampling_length_percent)))

    for b in range(B):
        points = list()

        if first_frame_only:
            valid_idxs = [0] if video[b, 0].reshape(-1).any() else []
        else:
            non_empty = video[b, :max_t].reshape(max_t, -1).any(dim=1)
            valid_idxs = non_empty.nonzero(as_tuple=True)[0].tolist()
        ...
        for _ in range(num_sampled_frames):
            frame_num = random.choice(valid_idxs)
        ...
        if len(points) < size:  
            pad = [[-1, -1, -1]] * (size - len(points))
            points.extend(pad)

        all_points.append(points[:size])
    return torch.tensor(all_points, device=device)
