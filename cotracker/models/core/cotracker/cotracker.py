...
class EfficientUpdateFormer(nn.Module):
    ...
    def __init__(
        ...
        # Temporal mixer: SSM replaces self-attention on the time axis when
        # use_ssm=True (TAPNext-style separation: SSM for time, attention
        # for space). Otherwise use the original temporal self-attention.
        self.time_depth = time_depth
        if use_ssm:
            self.ssm_time_blocks = nn.ModuleList(
                [
                    SSMBlock(
                        hidden_size,
                        ...
                        mlp_ratio=mlp_ratio,
                        bidirectional=ssm_bidirectional,
                    )
        ...
        if add_space_attn:
            ...
            assert time_depth >= space_depth
        self.initialize_weights()

    def initialize_weights(self):
        # Collect parameters inside SSM blocks so we can skip them.
        # Mamba has critical custom initialization (dt_proj bias holds
        # softplus-inverted timesteps; A_log holds negative log eigenvalues)
        # that xavier_uniform_ + zero-bias would destroy.
        ssm_param_ids = set()
        if self.use_ssm:
            for block in self.ssm_time_blocks:
                for p in block.ssm_fwd.parameters():
                    ssm_param_ids.add(id(p))
                if block.bidirectional:
                    for p in block.ssm_rev.parameters():
                        ssm_param_ids.add(id(p))

        def _basic_init(module):
            if isinstance(module, nn.Linear):
                if id(module.weight) in ssm_param_ids:
                    return
    ...
    def forward(self, input_tensor, mask=None, add_space_attn=True):
        ...
        j = 0
        layers = []
        # Temporal mixer: SSM (when use_ssm=True) OR self-attention, not both.
        time_mixer_blocks = self.ssm_time_blocks if self.use_ssm else self.time_blocks
        for i in range(self.time_depth):
            time_tokens = tokens.contiguous().view(B * N, T, -1)  # B N T C -> (B N) T C
            time_tokens = time_mixer_blocks[i](time_tokens)

            tokens = time_tokens.view(B, N, T, -1)  # (B N) T C -> B N T C
            if (
                add_space_attn
                and hasattr(self, "space_virtual_blocks")
                and (i % (self.time_depth // len(self.space_virtual_blocks)) == 0)
            ):
                space_tokens = (
...
class CrossAttnBlock(nn.Module):
    def __init__(
        self, hidden_size, context_dim, num_heads=1, mlp_ratio=4.0,
        attn_class=AttentionTorch, **block_kwargs,
    ):
        super().__init__()
