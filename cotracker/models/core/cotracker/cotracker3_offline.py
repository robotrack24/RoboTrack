...
def _mem_log(label: str):
    """Log current CUDA memory if available and debug flag is set."""
    if not (torch.cuda.is_available() and _mem_log.enabled):
        return
    alloc = torch.cuda.memory_allocated() / 1024**3
    reserved = torch.cuda.memory_reserved() / 1024**3
    logging.info(f"[MEM] {label:.<50s} alloc={alloc:6.2f} GB  reserved={reserved:6.2f} GB")

_mem_log.enabled = False
...
class CoTrackerThreeOffline(CoTrackerThreeBase):
    ...
    def forward(
        ...
        _mem_log(f"forward start (B={B} T={T} N={N} H={H} W={W})")
        # B = batch size
        ...
        # Check if encoder supports video input (e.g., VGGT with cross-frame attention)
        supports_video = getattr(self.fnet, 'supports_video_input', False)
        ...
        if T > fmaps_chunk_size:
            ...
            for t in range(0, T, fmaps_chunk_size):
                ...
                if supports_video:
                    # Pass video chunk with temporal dimension preserved
                    fmaps_chunk = self.fnet(video_chunk)  # (B, T_chunk, C, H, W) -> (B, T_chunk, out_dim, H4, W4)
                    fmaps_chunk = fmaps_chunk.view(-1, *fmaps_chunk.shape[2:])  # Flatten to (B*T_chunk, out_dim, H4, W4)
                else:
            ...
            if supports_video:
                # Pass full video with temporal dimension preserved
                fmaps = self.fnet(video)  # (B, T, C, H, W) -> (B, T, out_dim, H4, W4)
                fmaps = fmaps.view(-1, *fmaps.shape[2:])  # Flatten to (B*T, out_dim, H4, W4)
            else:
        ...
        _mem_log(f"after encoder (fmaps {tuple(fmaps.shape)})")
        ...
        max_pool_chunk = max(1, (2**31 - 1) // (self.latent_dim * fmaps.shape[-2] * fmaps.shape[-1]))
        for i in range(self.corr_levels - 1):
            BT = B * T
            if BT <= max_pool_chunk:
                fmaps_ = F.avg_pool2d(
                    fmaps.reshape(BT, self.latent_dim, fmaps.shape[-2], fmaps.shape[-1]),
                    2, stride=2,
                )
            else:
                fmaps_flat = fmaps.reshape(BT, self.latent_dim, fmaps.shape[-2], fmaps.shape[-1])
                chunks = []
                for t0 in range(0, BT, max_pool_chunk):
                    chunks.append(F.avg_pool2d(fmaps_flat[t0:t0 + max_pool_chunk], 2, stride=2))
                fmaps_ = torch.cat(chunks, dim=0)
        ...
        _mem_log(f"after fmaps pyramid ({len(fmaps_pyramid)} levels)")
        ...
        for it in range(iters):
            ...
            for i in range(self.corr_levels):
                ...
                _mem_log(f"  iter {it} level {i} after get_correlation_feat")
                track_feat_support = (
                ...
                _mem_log(f"  iter {it} level {i} after einsum")
                del corr_feat
                corr_emb = self.corr_mlp(corr_volume.reshape(B * T * N, r * r * r * r))
                del corr_volume
                _mem_log(f"  iter {it} level {i} after corr_mlp + cleanup")
            ...
            _mem_log(f"iter {it} before updateformer")
            delta = self.updateformer(
                x,
                add_space_attn=add_space_attn,
            )
            _mem_log(f"iter {it} after updateformer")
