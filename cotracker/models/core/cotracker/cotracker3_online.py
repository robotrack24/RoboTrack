...
from cotracker.models.core.cotracker.blocks import Mlp, BasicEncoder, Attention, AttentionTorch
...
class CoTrackerThreeBase(nn.Module):
    def __init__(
        ...
        encoder=None,  # Optional encoder config (DictConfig) or None for default
        normalize_input=True,  # Set False for encoders that handle their own normalization (e.g. VGGT)
        hidden_size=384,
        space_depth=3,
        time_depth=3,
        mlp_ratio=4.0,
        ...
        attn_class = AttentionTorch if use_flash_attn else Attention
        
        # Build encoder: use config if provided, otherwise default BasicEncoder
        if encoder is not None and isinstance(encoder, DictConfig):
            self.fnet = instantiate(encoder)
            # Update latent_dim if encoder specifies output_dim
            if hasattr(self.fnet, 'output_dim'):
                self.latent_dim = self.fnet.output_dim
        else:
        ...
        self.corr_mlp = Mlp(in_features=49 * 49, hidden_features=hidden_size, out_features=256)
...
class CoTrackerThreeOnline(CoTrackerThreeBase):
    ...
    def forward_window(
        ...
        for it in range(iters):
            coords = coords.detach().contiguous()  # B T N 2
