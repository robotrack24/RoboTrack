...
class EvaluationPredictor(torch.nn.Module):
    def __init__(
        ...
        self.is_online = isinstance(cotracker_model, CoTrackerThreeOnline)
        self.model.eval()
    ...
    def forward(self, video, queries):
        ...
        if self.single_point:
            ...
            if self.is_online:
                preds = self._forward_online(video, queries)
            else:
    ...
    def _forward_online(self, video, queries):
        """Stream the full video through the online model and return final predictions.

        Handles init + chunked forward passes, matching the protocol used by
        CoTrackerOnlinePredictor but keeping everything in interp-pixel space
        so the caller can rescale uniformly.
        """
        B, T, C, H, W = video.shape
        step = self.model.window_len // 2

        self.model.init_video_online_processing()
        traj_e = vis_e = conf_e = None
        for chunk_start in range(0, T - step, step):
            chunk = video[:, chunk_start : chunk_start + step * 2]
            traj_e, vis_e, conf_e, __ = self.model(
                video=chunk, queries=queries, iters=self.n_iters, is_online=True,
            )
        return traj_e, vis_e, conf_e, None
